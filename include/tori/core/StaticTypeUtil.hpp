...
namespace TORI_NS::detail {
  ...
  /// apply static_assert for result
  template <class T>
  struct unify_assert
  {
    using type = T;
  ...
  template <class Var, class Other>
  struct unify_assert<error_type<error_tags::circular_constraints<Var, Other>>>
  {
  ...
  /// generate new type variables for polymorphic term
  ...
  template <class T>
  struct is_var_type_impl : std::false_type
  {
  };

  template <class Tag>
  struct is_var_type_impl<var<Tag>> : std::true_type
  {
  };

  /// is_vartype_v
  template <class T>
  static constexpr bool is_var_type_v = is_var_type_impl<T>::value;
