...
namespace Neco::lang {
  ...
  template <class T, template <class> class AllocatorTemplate = std::allocator>
  ...
  /// Destructor
  /// \effects Destroy object with vtable function when reference count become 0
  template <class T>
  ObjectPtr<T>::~ObjectPtr() noexcept {
    if (m_ptr && m_ptr->refcount.atomic != 0 && --(m_ptr->refcount.atomic) == 0)
