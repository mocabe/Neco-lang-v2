...
void test_unify()
{
  ...
  }
  {
    // [X = int, Y = X -> X]
    using c =
      std::tuple<constr<var<class X>, value<int>>,
                 constr<var<class Y>, arrow<var<class X>, var<class X>>>>;
    static_assert(
      set_eq_v<std::tuple<tyarrow<var<class X>, value<int>>,
  ...
  }
  {
    ...
    static_assert(set_eq_v<std::tuple<tyarrow<var<class X>, value<int>>,
...
void test_type_of()
{
  ...
  }
  {
    ...
    using tm1 =
      tm_apply<tm_closure<tm_closure<tm_value<double>, tm_var<class X>>,
                          tm_var<class X>>,
               tm_closure<tm_value<double>, tm_value<int>>>;
    static_assert(std::is_same_v<value<int>, type_of_t<tm1>>);
  }
  {
    ...
    using doubleapp = tm_closure<tm_closure<tm_var<class X>, tm_var<class X>>,
  ...
  }
  {
    ...
    using ff = tm_apply<tm_fix<Fix>,
                        tm_closure<tm_closure<tm_value<int>, tm_value<bool>>,
                                   tm_value<int>,
                                   tm_value<bool>>>;
    static_assert(
...
void test_genpoly()
{
  {
    // Double -> Var<X> -> Var<X>
    using term = remove_varvalue_t<
      tm_closure<tm_closure<tm_value<double>, tm_varvalue<class X>>,
                 tm_varvalue<class X>>>;
    // Double -> Var[0] -> Var[0]
    using gterm = tm_closure<tm_closure<tm_value<double>, tm_var<taggen<0>>>,
                             tm_var<taggen<0>>>;
...
void test_assume_object_type()
{
  ...
  }
  {
    ...
    static_assert(
      std::is_same_v<assume_object_type_t<arrow<value<Double>, value<Int>>>,
                     closure<Double, Int>>);
    // arrow<S, arrow<T, U>> -> closure<S, T, U>
    static_assert(
      std::is_same_v<assume_object_type_t<
    ...
    static_assert(std::is_same_v<
                  assume_object_type_t<arrow<arrow<value<Double>, value<Int>>,
